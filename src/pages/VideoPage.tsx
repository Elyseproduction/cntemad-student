...
import { Plus, Trash2, X, Play, Video, Filter, ChevronLeft, ChevronRight, Calendar, BookOpen, Maximize2, Minimize2, Upload, Loader2 } from 'lucide-react';
...
export function VideoPage() {
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!file.type.startsWith('video/')) {
      toast({ title: 'Fichier invalide', description: 'Veuillez sélectionner un fichier vidéo.', variant: 'destructive' });
      return;
    }

    if (file.size > 500 * 1024 * 1024) {
      toast({ title: 'Fichier trop volumineux', description: 'Maximum 500 Mo par vidéo.', variant: 'destructive' });
      return;
    }

    if (!newTitle.trim()) {
      toast({ title: 'Titre requis', description: 'Veuillez entrer un titre pour la vidéo.', variant: 'destructive' });
      return;
    }
    ...
    try {
      ...
      toast({ title: '✅ Vidéo uploadée !', description: 'Votre vidéo est maintenant disponible pour tous.' });
      resetForm();
    } catch (err: any) {
  ...
  const isLocalVideo = (v: typeof videos[0]) => v.type === 'local' || !!v.videoUrl;

  const getVideoThumbnail = (video: typeof videos[0]) => {
    if (isLocalVideo(video)) return null;
  ...
  const handleVideoClick = (videoId: string) => {
    ...
    if (!video) return;
    if (isAndroid && !isLocalVideo(video)) {
  ...
  return (
    <div className="max-w-6xl mx-auto animate-fade-in">
        ...
        <div className="glass-card p-6 mb-6 animate-scale-in">
          ...
          <div className="flex gap-2 mb-4">
            <button
              ...
              className={`flex-1 py-2.5 rounded-lg text-sm font-medium transition-all flex items-center justify-center gap-2 ${
                addMode === 'youtube' ? 'bg-primary/15 text-primary border border-primary/20' : 'bg-secondary text-muted-foreground'
              }`}
            >
            ...
            <button
              ...
              className={`flex-1 py-2.5 rounded-lg text-sm font-medium transition-all flex items-center justify-center gap-2 ${
                addMode === 'upload' ? 'bg-primary/15 text-primary border border-primary/20' : 'bg-secondary text-muted-foreground'
              }`}
            >
              <Upload size={16} /> Depuis mon appareil
            </button>
          </div>
          ...
          <div className="space-y-3">
            ...
            {addMode === 'upload' && (
              <>
                {uploading ? (
                  ...
                  <label className="flex flex-col items-center justify-center w-full h-32 rounded-xl border-2 border-dashed border-primary/30 bg-primary/5 hover:bg-primary/10 cursor-pointer transition-colors">
                    <Upload className="w-8 h-8 text-primary/60 mb-2" />
                    <span className="text-sm font-medium text-foreground">Sélectionner une vidéo</span>
                    <span className="text-xs text-muted-foreground mt-1">MP4, MOV, WebM (max 500 Mo)</span>
                    <input
                      ref={fileInputRef}
                      type="file"
                      accept="video/*"
            ...
            <div className="flex gap-3">
              <button onClick={resetForm} className="flex-1 py-2 rounded-lg bg-secondary text-foreground">Annuler</button>
              {addMode === 'youtube' && (
                <button onClick={handleAddYoutube} className="flex-1 py-2 rounded-lg gradient-bg text-primary-foreground font-medium">➕ Ajouter</button>
              )}
            </div>
          </div>
        </div>
          ...
          <div key={video.id} className="glass-card overflow-hidden animate-slide-up hover:shadow-md transition-shadow" style={{ animationDelay: `${i * 0.05}s` }}>
            <div className="relative cursor-pointer group" onClick={() => handleVideoClick(video.id)}>
              {isLocalVideo(video) ? (
                <div className="w-full aspect-video bg-muted/50 flex items-center justify-center">
                  <div className="text-center">
                    <Video size={40} className="mx-auto text-primary/40 mb-1" />
                    <span className="text-xs text-muted-foreground">Vidéo locale</span>
                  </div>
                </div>
              ) : (
                <img
              ...
              <div className="absolute bottom-2 right-2 bg-background/80 backdrop-blur-sm text-foreground text-xs px-2 py-0.5 rounded font-medium">
                {isLocalVideo(video) ? '📱 Local' : 'YouTube'}
              </div>
            </div>
            <div className="p-4">
      ...
      {selectedVideo && (() => {
        ...
        const isLocal = isLocalVideo(video);
        return (
          <div className="fixed inset-0 z-50 flex flex-col animate-fade-in" style={{ animationDuration: '0.2s' }}>
            ...
            <div className="relative z-10 flex items-center justify-between px-4 md:px-6 py-3 border-b border-border/50">
              <div className="flex items-center gap-3 min-w-0">
                <button onClick={() => setSelectedVideo(null)} className="p-2 rounded-lg hover:bg-secondary transition-colors flex-shrink-0">
              ...
              <div className="flex items-center gap-2 flex-shrink-0">
                <div className="hidden sm:flex items-center gap-1 mr-2">
                  <span className="text-xs text-muted-foreground">{currentVideoIndex + 1} / {sortedVideos.length}</span>
                </div>
                <button onClick={() => navigateVideo('prev')} disabled={!hasPrev} className="p-2 rounded-lg hover:bg-secondary transition-colors disabled:opacity-30"><ChevronLeft size={18} /></button>
                <button onClick={() => navigateVideo('next')} disabled={!hasNext} className="p-2 rounded-lg hover:bg-secondary transition-colors disabled:opacity-30"><ChevronRight size={18} /></button>
                <button onClick={() => setIsTheaterMode(p => !p)} className="p-2 rounded-lg hover:bg-secondary transition-colors hidden md:flex">
              ...
              <div className={`w-full transition-all duration-300 ${isTheaterMode ? 'max-w-full' : 'max-w-5xl'}`}>
                ...
                {hasNext && (
                  <div className="mt-4">
                    ...
                    >
                      {isLocalVideo(sortedVideos[currentVideoIndex + 1]) ? (
                        <div className="w-28 md:w-36 aspect-video rounded-lg bg-muted flex items-center justify-center flex-shrink-0">
